...
static void bacnet_task(void *pv) {
    ...
    static uint8_t next_station, poll_station;
    uart_event_t event;

    z_log(LOG_INFO, "BACNET", "Engine %s - TSM Enabled\n", VERSION_GLOBAL);
                        ...
                        if (validate_rx_header_crc(header)) {
                            ...
                            if (src_mac < 128 && src_mac != sysCfg.mac_address) {
                                if (xSemaphoreTake(cache_mutex, 0)) {
                                    bool known = false;
                                    for(auto& d : bacnet_network_cache) { if(d.mac_address == src_mac) { known = true; break; } }
                                    if(!known) {
                                        ...
                                        d.disc_step = DISC_DEV_ID; d.last_seen = millis();
        ...
        switch (mstp_state) {
            ...
            case MSTP_IDLE:
                if (ReceivedValidFrame) {
                    if (frame_type == 0x00 && dest_mac == sysCfg.mac_address) { 
                        bacnetStats.tokens_seen++; next_station = (src_mac + 1) % (sysCfg.max_master + 1); 
                        frame_count = 0; mstp_state = MSTP_USE_TOKEN; 
                    }
                    else if (frame_type == 0x01 && dest_mac == sysCfg.mac_address) { 
                        uint8_t f[8] = { 0x55, 0xFF, 0x02, src_mac, sysCfg.mac_address, 0, 0, 0 }; 
                        f[7] = calc_header_crc(&f[2], 5); uart_tx(f, 8); mstp_state = MSTP_IDLE; 
                    }
            ...
            case MSTP_USE_TOKEN:
                ...
                if (token_skip_count >= sysCfg.token_skip) { 
                    ...
                    uint8_t apdu[256]; uint16_t apdu_len = 0; uint8_t target_mac = 0;
                    if (has_job && current_job.type == JOB_WHO_IS) {
                        uint8_t buffer[16]; uint16_t len = 0;
                        buffer[len++] = 0x01; buffer[len++] = 0x20; buffer[len++] = 0xFF; buffer[len++] = 0xFF; buffer[len++] = 0x00; buffer[len++] = 0xFF; buffer[len++] = 0x10; buffer[len++] = 0x08;
                        send_mstp_frame(0xFF, 0x06, buffer, len);
                        token_skip_count = 0; frame_count++; mstp_state = MSTP_DONE_WITH_TOKEN;
                        ...
                        token_skip_count = 0; waiting_for_reply = true; send_mstp_frame(current_job.target_mac, 0x05, apdu, apdu_len);
                                ...
                                if (dev.enabled) {
                                    if (!dev.discovery_done) {
                                        if (dev.disc_step == DISC_DEV_ID) apdu_len = build_read_property_apdu(apdu, next_invoke_id++, 8, 4194303, 75, -1);
                                            ...
                                            z_log(LOG_INFO, "BACNET", "Discovery Done for %lu\n", dev.device_id);
                                            publish_all_names();
                                        }
                                    } else {
            ...
            case MSTP_WAIT_FOR_REPLY:
                if (ReceivedValidFrame) {
                    ...
                    if (data_len >= 2 && data_buf[0] == 0x01) {
                        uint16_t pos = 2; uint8_t *apdu = &data_buf[pos]; uint16_t apdu_len = data_len - pos; uint8_t pdu_type = apdu[0] & 0xF0;
                        if (dest_mac == sysCfg.mac_address) {
                            if (pdu_type == 0x20) { z_log(LOG_INFO, "BACNET", "Write OK\n"); } 
                                                ...
                                                if (current_dev_idx < bacnet_network_cache.size()) {
                                                    ...
                                                    while (apdu_pos < apdu_len && decode_next_tag(apdu, &apdu_pos, apdu_len, &val_tag)) {
                                                        ...
                                                        if (!dev.discovery_done) {
                                                            ...
                                                            else if (dev.disc_step == DISC_DEV_NAME) { char n[33]; uint8_t enc = apdu[apdu_pos]; if (enc == 0) { uint16_t slen = std::min((int)val_tag.len - 1, 32); memcpy(n, &apdu[apdu_pos+1], slen); n[slen]=0; } dev.name = String(n); dev.disc_step = DISC_DEV_VENDOR; }
                                                            ...
                                                            else if (dev.disc_step == DISC_OBJ_COUNT) { uint32_t count = 0; for(int i=0; i<val_tag.len; i++) count = (count << 8) | apdu[apdu_pos+i]; dev.disc_obj_idx = 0; dev.disc_step = DISC_OBJ_OID; }
                                                            else if (dev.disc_obj_idx < dev.objects.size()) {
                                                                auto& o = dev.objects[dev.disc_obj_idx];
                                                                if (dev.disc_step == DISC_OBJ_OID) { uint32_t oid = (apdu[apdu_pos]<<24)|(apdu[apdu_pos+1]<<16)|(apdu[apdu_pos+2]<<8)|apdu[apdu_pos+3]; o.type = oid >> 22; o.instance = oid & 0x3FFFFF; dev.disc_step = DISC_OBJ_NAME; }
                                                                else if (dev.disc_step == DISC_OBJ_NAME) { char n[33]; uint8_t enc = apdu[apdu_pos]; if (enc == 0) { uint16_t slen = std::min((int)val_tag.len - 1, 32); memcpy(n, &apdu[apdu_pos+1], slen); n[slen]=0; } strlcpy(o.name, n, 50); dev.disc_step = DISC_OBJ_UNITS; }
                                                                    ...
                                                                    o.last_update = millis(); dev.disc_obj_idx++; dev.disc_step = DISC_OBJ_OID;
                                                                    if (dev.disc_obj_idx >= dev.objects.size()) { dev.discovery_done = true; save_device_objects_locked(dev.device_id); publish_all_names(); }
                                                                }
                                                            }
                                                        } else {
                                                            if (val_tag.number == 4) { 
                                                                ...
                                                                if(current_poll_idx < dev.objects.size()){ 
                                                                    auto& o=dev.objects[current_poll_idx]; o.present_value=v; o.last_update=millis(); 
                                                                    z_log(LOG_DEBUG, "BACNET", "Recv Dev %lu Obj %u.%lu: %.2f\n", (unsigned long)dev.device_id, o.type, (unsigned long)o.instance, v);
                                                                ...
                                                                if(current_poll_idx < dev.objects.size()){ 
                                                                    auto& o=dev.objects[current_poll_idx]; o.present_value=(float)v; o.last_update=millis(); 
                                                                    z_log(LOG_DEBUG, "BACNET", "Recv Dev %lu Obj %u.%lu: %.2f\n", (unsigned long)dev.device_id, o.type, (unsigned long)o.instance, (float)v);
                                                                    if (o.enabled) publish_mqtt_topic(dev.device_id, o, 85, false); 
                                                                } 
                                                            }
                                                        }
                ...
                } else if (millis() - timer_silence > sysCfg.apdu_timeout) mstp_state = MSTP_DONE_WITH_TOKEN;
                break;
...
void setup_bacnet_engine() {
    ...
    z_log(LOG_INFO, "BACNET", "Engine Initialized\n");
}
    ...
    for (auto& dev : bacnet_network_cache) {
        for (auto& obj : dev.objects) {
            ...
            if (!obj.name_published || (strcmp(obj.name, obj.last_mqtt_name) != 0)) {
                ...
                strlcpy(obj.last_mqtt_name, obj.name, 50);
                z_log(LOG_DEBUG, "BACNET", "Sync MQTT Name: %s\n", obj.name);
            }
        }
    }
}
