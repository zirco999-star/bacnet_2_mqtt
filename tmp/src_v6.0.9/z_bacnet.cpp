...
BACnet_Stats bacnetStats = {0, 0, 0, 0, 0, 0, 0, 0};
...
static bool decode_next_tag(const uint8_t *data, uint16_t *pos, uint16_t max_len, BACnetTag *tag) {
    ...
    if (lvt <= 4) tag->len = lvt;
        ...
        tag->len = data[(*pos)++];
        if (tag->len == 254) { if(*pos+2 <= max_len) { tag->len = (data[*pos] << 8) | data[*pos+1]; *pos += 2; } }
        else if (tag->len == 255) { if(*pos+4 <= max_len) { tag->len = ((uint32_t)data[*pos] << 24) | ((uint32_t)data[*pos+1] << 16) | ((uint32_t)data[*pos+2] << 8) | data[*pos+3]; *pos += 4; } }
    } else tag->len = 0;
    return true;
}
...
uint16_t build_read_property_apdu(uint8_t* buffer, uint8_t invoke_id, uint16_t obj_type, uint32_t obj_instance, uint8_t property_id, int32_t array_index) {
    ...
    buffer[len++] = 0x0C; buffer[len++] = (oid >> 24) & 0xFF; buffer[len++] = (oid >> 16) & 0xFF; buffer[len++] = (oid >> 8) & 0xFF; buffer[len++] = oid & 0xFF;
...
static void bacnet_task(void *pv) {
    ...
    z_log(LOG_INFO, "BACNET", "Engine %s - TSM Enabled (v6.0.8 Final)\n", VERSION_GLOBAL);
    ...
    for (;;) {
        ...
        uint32_t who_is_interval = (bacnet_network_cache.empty()) ? 5000 : 300000;
        ...
        switch (mstp_state) {
            ...
            case MSTP_USE_TOKEN:
                ...
                if (token_skip_count >= sysCfg.token_skip) { 
                    ...
                    if (has_job && current_job.type == JOB_WHO_IS) {
                        uint8_t buffer[16]; uint16_t len = 0; buffer[len++] = 0x01; buffer[len++] = 0x00; buffer[len++] = 0x10; buffer[len++] = 0x08;
                        send_mstp_frame(0xFF, 0x06, buffer, len); timer_silence = millis(); token_skip_count = 0; frame_count++; mstp_state = MSTP_DONE_WITH_TOKEN;
                        ...
                        send_mstp_frame(target_mac, 0x05, apdu, apdu_len); timer_silence = millis(); frame_count++; mstp_state = MSTP_WAIT_FOR_REPLY;
                    } else {
                            ...
                            if (!bacnet_network_cache.empty()) {
                                ...
                                if (dev.enabled) {
                                    if (!dev.discovery_done) {
                                        if (dev.disc_step == DISC_DEV_ID) apdu_len = build_read_property_apdu(apdu, next_invoke_id++, 8, 4194303, 75, -1);
                                        else if (dev.device_id == 4194303) { /* Wait for I-Am */ }
                                            ...
                                            if (dev.disc_step == DISC_OBJ_OID) apdu_len = build_read_property_apdu(apdu, next_invoke_id++, 8, dev.device_id, 76, dev.disc_obj_idx + 1);
                                                ...
                                                if (dev.disc_step == DISC_OBJ_NAME) apdu_len = build_read_property_apdu(apdu, next_invoke_id++, o.type, o.instance, 77, -1);
                                                    ...
                                                    if(o.type <= 2 || o.type == 23 || o.type == 24 || o.type == 46) apdu_len = build_read_property_apdu(apdu, next_invoke_id++, o.type, o.instance, 117, -1);
                                                    else dev.disc_step = DISC_OBJ_STATES;
                                                }
                                                if (dev.disc_step == DISC_OBJ_STATES) {
                                                    if (o.type == 13 || o.type == 14 || o.type == 19) {
                                                        if (o.expected_states_count == 0) apdu_len = build_read_property_apdu(apdu, next_invoke_id++, o.type, o.instance, 110, 0);
                                                        else apdu_len = build_read_property_apdu(apdu, next_invoke_id++, o.type, o.instance, 110, o.state_texts.size() + 1);
                                                    } else dev.disc_step = DISC_OBJ_VALUE;
                                                }
                                                if (dev.disc_step == DISC_OBJ_VALUE) {
                                                    if(o.type <= 5 || (o.type >= 13 && o.type <= 19)) apdu_len = build_read_property_apdu(apdu, next_invoke_id++, o.type, o.instance, 85, -1);
                                        ...
                                        if (count > 0) {
                                            for (size_t i = 0; i < count; i++) {
                                                ...
                                                if (o.enabled && o.type != 8 && (o.last_update == 0 || (millis() - o.last_update > (sysCfg.bacnet_poll_interval * 1000)))) { apdu_len = build_read_property_apdu(apdu, next_invoke_id++, o.type, o.instance, 85, -1); break; }
                                            }
                                        }
                                    }
                                }
            ...
            case MSTP_WAIT_FOR_REPLY:
                if (ReceivedValidFrame) {
                    ...
                    if (frame_type == 0x07) { mstp_state = MSTP_DONE_WITH_TOKEN; break; }
                    if (data_len >= 2 && data_buf[0] == 0x01) {
                        uint8_t ctrl = data_buf[1]; uint16_t pos = 2;
                        if (ctrl & 0x20) { if (pos + 2 < data_len) { uint8_t dlen = data_buf[pos + 2]; pos += 3 + dlen; } }
                        if (ctrl & 0x08) { if (pos + 2 < data_len) { uint8_t slen = data_buf[pos + 2]; pos += 3 + slen; } }
                        if (ctrl & 0x20) pos += 1;
                        if (pos < data_len) {
                            uint8_t *apdu = &data_buf[pos]; uint16_t apdu_len = data_len - pos;
                            uint8_t pdu_type = apdu[0] & 0xF0; uint8_t invoke_id = apdu[1];
                            if (dest_mac == sysCfg.mac_address && (invoke_id == last_sent_invoke_id || pdu_type == 0x10)) {
                                if (pdu_type == 0x10) { // I-Am
                                    // Tag 0 (C4) at apdu[2], OID at apdu[3..6]
                                    uint32_t dev_id = (((uint32_t)apdu[3] << 24) | ((uint32_t)apdu[4] << 16) | ((uint32_t)apdu[5] << 8) | apdu[6]) & 0x3FFFFF; 
                                    if (xSemaphoreTake(cache_mutex, 50)) { 
                                        for(auto& d : bacnet_network_cache) { 
                                            if(d.mac_address == src_mac) {
                                                if (d.device_id != dev_id) {
                                                    z_log(LOG_INFO, "BACNET", "MAC %u I-Am: ID %lu\n", src_mac, (unsigned long)dev_id);
                                                    d.device_id = dev_id;
                                                    if (d.disc_step == DISC_DEV_ID) d.disc_step = DISC_DEV_NAME;
                                                }
                                            }
                                        } 
                                        xSemaphoreGive(cache_mutex); 
                                    }
                                } else if (pdu_type == 0x30) { // Complex-ACK
                                                    ...
                                                    while (apdu_pos < apdu_len && decode_next_tag(apdu, &apdu_pos, apdu_len, &val_tag)) {
                                                        ...
                                                        if (!dev.discovery_done) {
                                                            if (dev.disc_step == DISC_DEV_ID) { 
                                                                dev.device_id = (((uint32_t)apdu[apdu_pos] << 24) | ((uint32_t)apdu[apdu_pos+1] << 16) | ((uint32_t)apdu[apdu_pos+2] << 8) | apdu[apdu_pos+3]) & 0x3FFFFF; 
                                                                z_log(LOG_INFO, "BACNET", "MAC %u Identified: ID %lu\n", dev.mac_address, (unsigned long)dev.device_id);
                                                                dev.disc_step = DISC_DEV_NAME; 
                                                            }
                                                            else if (dev.disc_step == DISC_DEV_NAME) { char n[33]; uint8_t enc = apdu[apdu_pos]; if (enc == 0) { uint16_t slen = std::min((int)val_tag.len - 1, 32); memcpy(n, &apdu[apdu_pos+1], slen); n[slen]=0; } else { int slen=0; for(int i=2; i<(int)val_tag.len && slen<32; i+=2) n[slen++]=apdu[apdu_pos+i]; n[slen]=0; } dev.name = String(n); dev.disc_step = DISC_DEV_VENDOR; }
                                                            else if (dev.disc_step == DISC_DEV_VENDOR) { char n[33]; uint8_t enc = apdu[apdu_pos]; if (enc == 0) { uint16_t slen = std::min((int)val_tag.len - 1, 32); memcpy(n, &apdu[apdu_pos+1], slen); n[slen]=0; } else { int slen=0; for(int i=2; i<(int)val_tag.len && slen<32; i+=2) n[slen++]=apdu[apdu_pos+i]; n[slen]=0; } dev.vendor = String(n); dev.disc_step = DISC_OBJ_COUNT; }
                                                            else if (dev.disc_step == DISC_OBJ_COUNT) { uint32_t count = 0; for(int i=0; i<(int)val_tag.len; i++) count = (count << 8) | apdu[apdu_pos+i]; if (count > 0 && count < 200) { dev.total_slots = count; dev.objects.clear(); dev.disc_obj_idx = 0; dev.disc_step = DISC_OBJ_OID; z_log(LOG_INFO, "BACNET", "Dev %lu: %lu slots detected\n", dev.device_id, count); } else dev.discovery_done = true; }
                                                            else if (dev.disc_obj_idx < dev.total_slots) {
                                                                if (dev.disc_step == DISC_OBJ_OID) { uint32_t oid = ((uint32_t)apdu[apdu_pos]<<24)|((uint32_t)apdu[apdu_pos+1]<<16)|((uint32_t)apdu[apdu_pos+2]<<8)|(uint32_t)apdu[apdu_pos+3]; dev.debug_oid = oid; if (oid != 0xFFFFFFFF && (oid >> 22) < 1023) { BACnetObject o; o.type = oid >> 22; o.instance = oid & 0x3FFFFF; if (o.type <= 5 || (o.type >= 13 && o.type <= 19)) o.enabled = true; snprintf(o.name, 50, "OBJ:%lu", (unsigned long)o.instance); dev.objects.push_back(o); dev.disc_step = DISC_OBJ_NAME; } else dev.disc_obj_idx++; }
                                                                else if (!dev.objects.empty()) {
                                                                    auto& o = dev.objects.back();
                                                                    if (dev.disc_step == DISC_OBJ_NAME) { char n[33]; uint8_t enc = apdu[apdu_pos]; if (enc == 0) { uint16_t slen = std::min((int)val_tag.len - 1, 32); memcpy(n, &apdu[apdu_pos+1], slen); n[slen]=0; } else { int slen=0; for(int i=2; i<(int)val_tag.len && slen<32; i+=2) n[slen++]=apdu[apdu_pos+i]; n[slen]=0; } if(strlen(n)>0) strlcpy(o.name, n, 50); dev.disc_step = DISC_OBJ_UNITS; }
                                                                    else if (dev.disc_step == DISC_OBJ_UNITS) { uint32_t u=0; for(int i=0; i<(int)val_tag.len; i++) u = (u << 8) | apdu[apdu_pos+i]; o.units = u; strlcpy(o.unit_text, get_unit_text(u).c_str(), 20); dev.disc_step = DISC_OBJ_STATES; }
                                                                    else if (dev.disc_step == DISC_OBJ_STATES) {
                                                                        if (val_tag.number == 2) { uint32_t c = 0; for(int i=0; i<(int)val_tag.len; i++) c = (c << 8) | apdu[apdu_pos+i]; o.expected_states_count = (uint16_t)c; if (o.expected_states_count == 0) dev.disc_step = DISC_OBJ_VALUE; }
                                                                        else { char n[33]; uint8_t enc = apdu[apdu_pos]; if (enc == 0) { uint16_t slen = std::min((int)val_tag.len - 1, 32); memcpy(n, &apdu[apdu_pos+1], slen); n[slen]=0; } else { int slen=0; for(int i=2; i<(int)val_tag.len && slen<32; i+=2) n[slen++]=apdu[apdu_pos+i]; n[slen]=0; } o.state_texts.push_back(String(n)); if (o.state_texts.size() >= o.expected_states_count) dev.disc_step = DISC_OBJ_VALUE; }
                                                                    }
                                                                    else if (dev.disc_step == DISC_OBJ_VALUE) { if (val_tag.number == 4) { float v; uint32_t tmp; memcpy(&tmp, &apdu[apdu_pos], 4); tmp = __builtin_bswap32(tmp); memcpy(&v, &tmp, 4); o.present_value = v; } else { uint32_t v=0; for(int i=0; i<(int)val_tag.len; i++) v = (v << 8) | apdu[apdu_pos+i]; o.present_value = (float)v; } o.last_update = millis(); dev.disc_obj_idx++; if (dev.disc_obj_idx >= dev.total_slots) { dev.discovery_done = true; save_device_objects_locked(dev.device_id); publish_all_names(); } else dev.disc_step = DISC_OBJ_OID; }
                                                                } else { dev.disc_obj_idx++; dev.disc_step = DISC_OBJ_OID; }
                                                            }
                                                        } else {
                                                            if (val_tag.number == 4) { float v; uint32_t tmp; memcpy(&tmp, &apdu[apdu_pos], 4); tmp = __builtin_bswap32(tmp); memcpy(&v, &tmp, 4); if(current_poll_idx < dev.objects.size()){ auto& obj=dev.objects[current_poll_idx]; obj.present_value=v; obj.last_update=millis(); if (obj.enabled) publish_mqtt_topic(dev.device_id, obj, 85, false); } }
                                                            else if (val_tag.number == 2 || val_tag.number == 9) { uint32_t v=0; for(int i=0; i<(int)val_tag.len; i++) v=(v<<8)|apdu[apdu_pos+i]; if(current_poll_idx < dev.objects.size()){ auto& obj=dev.objects[current_poll_idx]; obj.present_value=(float)v; obj.last_update=millis(); if (obj.enabled) publish_mqtt_topic(dev.device_id, obj, 85, false); } }
                                                        }
                                ...
                                } else if (pdu_type == 0x50) { // Error-PDU
                                    uint8_t err_c = 0, err_code = 0; if(apdu_len >= 7) { err_c = apdu[4]; err_code = apdu[6]; }
                                    if (xSemaphoreTake(cache_mutex, 50)) { 
                                        if (current_dev_idx < bacnet_network_cache.size()) { 
                                            auto& d = bacnet_network_cache[current_dev_idx]; 
                                            z_log(LOG_WARN, "BACNET", "Error Class %u Code %u (Step %d Idx %d OID %08lX)\n", err_c, err_code, d.disc_step, d.disc_obj_idx, (unsigned long)d.debug_oid);
                                            if (!d.discovery_done) { 
                                                if (d.disc_step >= DISC_OBJ_OID) {
                                                    d.disc_obj_idx++; d.disc_step = DISC_OBJ_OID; if (d.disc_obj_idx >= d.total_slots) d.discovery_done = true; 
                                                } else {
                                                    z_log(LOG_WARN, "BACNET", "Resetting step for MAC %u due to device-level error\n", d.mac_address);
                                                    d.disc_step = DISC_DEV_ID; // Retry device identification
                                                }
                                            } 
                                        } 
                                        xSemaphoreGive(cache_mutex); 
                    ...
                    if (xSemaphoreTake(cache_mutex, 50)) { if (current_dev_idx < bacnet_network_cache.size()) { auto& d = bacnet_network_cache[current_dev_idx]; if (!d.discovery_done) { retry_count++; if (retry_count > sysCfg.max_retries) { d.disc_obj_idx++; d.disc_step = DISC_OBJ_OID; retry_count = 0; if (d.disc_obj_idx >= d.total_slots) d.discovery_done = true; } } } xSemaphoreGive(cache_mutex); }
                    mstp_state = MSTP_DONE_WITH_TOKEN;
                }
                break;
